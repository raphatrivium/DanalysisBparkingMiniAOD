...
//======================================================
	...
	//For D* MC
	//-----------------------------------------
	//t_DsMC.Branch("dScandsKpi",&var_dScandsKpi, "var_dScandsKpi/D");
	...
	for (Long64_t jentry=0; jentry < nentries; jentry++) //loop tree entries for file f1
	{
      ...
      if (ientry < 0) break;
		...
		{  if (debug)cout << "For D* MC loop --------------------"<< endl;
			...
			//var_dScandsKpi = dScandsKpi->at(i);
			...
			if( deltaR < 0.03){
				...
				recTime = (Dsmass->at(id)*D0fromDSdXY->at(id)*pow(10,-2)) / (c*Dspt->at(id)); 
		...
		{  if (debug)cout << "debug D* 22 --------------------"<< endl;
			//var_dScandsKpi = dScandsKpi->at(h1);
