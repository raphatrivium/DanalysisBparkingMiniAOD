...
//======================================================
	...
	for (Long64_t jentry=0; jentry < nentries; jentry++) //loop tree entries for file f1
	{
      ...
      if (ientry < 0) break;
		...
		for(unsigned int i=0; i < MCDsmass->size(); i++) // loop D* MC
		{  if (debug)cout << "For D* MC loop --------------------"<< endl;
			...
			//For Ds Matching
			//-----------------------------------------
			if ( D0mass->size() > 0){ // protection matching
		...
		//For D0 MC
		//-----------------------------------------
		if(MCpromptD0mass->size() > 0 && FlagD0MC){ // protection D0 MC
		for(unsigned int i=0; i < MCpromptD0mass->size(); i++) // loop D0 MC
		{  if (debug)cout << "debug D* 22 --------------------"<< endl;
			...
			//----------------------------------------
			if ( D0Kpimass->size() > 0){ // D0 Matching protection
				double minDeltaR = 1000.;
				int id = -1;
				double deltaR = 0.;
	...
	if (debug)cout << "debug 26 --------------------"<< endl;
	
	if (Dataset == "MC_DStarToD0Pi_D0KPi" or Dataset == "MC_MinBias"){
		if (debug)cout << "debug 27 --------------------"<< endl;
	...
	cout << "--------------------"<< endl;
		if (debug)cout << "debug 28 --------------------"<< endl;
	cout << "----------  C O N T A M I N A T I O N   D * -------------"<< endl;
	cout << "Total / Dstar Prompt / Dstar Secondary / Contamination  ----------"<< endl;
	int DstarPrompt = DstarTotal-DstarSec ;
	int Dstarcontamination = DstarSec / DstarTotal;
		if ( DstarTotal > 0 ){ 
				cout << DstarTotal << " & " << DstarPrompt << " & " << DstarSec << " & " << Dstarcontamination << endl;
			}
	}
	
		if (debug)cout << "debug 29 --------------------"<< endl;
...
//----------------------------------------------------------------------------------------------------------------
	...
	D0KpimassPtHisto9->Write();
		if (debug)cout << "debug 43 --------------------"<< endl;
#if GENvaraibles == 0	
#elif GENvaraibles == 1
			if (debug)cout << "debug 44 --------------------"<< endl;
	t_DsMC.Write(); //Write() if a file is open, this function writes a root objectics on it.
	t_D0MC.Write();
	t_DsMatching.Write();
	t_D0Matching.Write();
			if (debug)cout << "debug 45 --------------------"<< endl;
	hdeltaRDstar->Write();
	...
	hDspt->Write();
	hDslifetime->Write();
		if (debug)cout << "debug 49 --------------------"<< endl;
	hDsMinusD0->Write();
	hD0mass->Write();
	hD0eta->Write();
	hD0phi->Write();
	hD0pt->Write();
		if (debug)cout << "debug 50 --------------------"<< endl;
	hTrkKpt->Write();
	hTrkKchi2->Write();
	hTrkKnhits->Write();
	hTrkKdz->Write();
